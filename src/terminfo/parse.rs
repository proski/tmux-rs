...
fn read_number16(reader: &mut Cursor<&[u8]>) -> Result<Option<i32>, TerminfoError> {
    let mut buffer = [0u8; 2];
    reader.read_exact(&mut buffer)?;
    let value = i32::from(i16::from_le_bytes(buffer));
...
fn read_number32(reader: &mut Cursor<&[u8]>) -> Result<Option<i32>, TerminfoError> {
...
pub fn parse(buffer: &[u8]) -> Result<Terminfo, TerminfoError> {
    ...
    let number_size = match TerminfoMagic::try_from(magic) {
        Ok(TerminfoMagic::Magic1) => 2,
        Ok(TerminfoMagic::Magic2) => 4,
        Err(_) => return Err(TerminfoError::UnsupportedFormat),
    };

    if bool_count > BOOL_NAMES.len() || num_count > NUM_NAMES.len() || str_count > STR_NAMES.len() {
    ...
    for name in BOOL_NAMES.iter().take(bool_count) {
        ...
        if value == 1 {
            booleans.insert(name.to_string(), true);
        }
    }

    align_cursor(&mut reader)?;

    let read_number = if number_size == 4 {
        read_number32
    } else {
        read_number16
    };
    for name in NUM_NAMES.iter().take(num_count) {
        if let Some(number) = read_number(&mut reader)? {
    ...
    for name in STR_NAMES.iter().take(str_count) {
        ...
        let index = &str_table[offset..].iter().position(|c| *c == b'\0');
        let Some(index) = index else {
            return Err(TerminfoError::UnterminatedString);
        };
        let value = (str_table[offset..offset + index]).to_vec();
    ...
    let ext_bools = read_slice(&mut reader, ext_bool_count)?;
    let mut ext_bools_reader = Cursor::new(ext_bools);
    align_cursor(&mut reader)?;

    let ext_nums = read_slice(&mut reader, number_size * ext_num_count)?;
    let mut ext_nums_reader = Cursor::new(ext_nums);

    let ext_strs = read_slice(&mut reader, std::mem::size_of::<u16>() * ext_str_count)?;
    ...
    loop {
        ...
        let end_index = &ext_str_table[offset..].iter().position(|c| *c == b'\0');
        let Some(end_index) = end_index else {
            return Err(TerminfoError::UnterminatedString);
        };
        names_base += end_index + 1;
    }
    ...
    loop {
        ...
        let value = match value {
            0 => false,
            1 => true,
            _ => return Err(TerminfoError::UnsupportedFormat),
        };
        ...
        let name_length = &names_table[name_offset..].iter().position(|c| *c == b'\0');
        let Some(name_length) = name_length else {
            return Err(TerminfoError::UnterminatedString);
        };
        let name = &names_table[name_offset..name_offset + name_length];
        booleans.insert(str::from_utf8(name)?.to_string(), value);
    }

    loop {
        let Ok(value) = read_number(&mut ext_nums_reader) else {
            break;
        };
        ...
        let name = &names_table[name_offset..name_offset + name_length];
        numbers.insert(str::from_utf8(name)?.to_string(), value);
    }
    ...
    loop {
        ...
        let str_length = &ext_str_table[str_offset..].iter().position(|c| *c == b'\0');
        let Some(str_length) = str_length else {
            return Err(TerminfoError::UnterminatedString);
        };
        let value = &ext_str_table[str_offset..str_offset + str_length];
        ...
        let name = &names_table[name_offset..name_offset + name_length];
        strings.insert(str::from_utf8(name)?.to_string(), value.to_vec());
    }
