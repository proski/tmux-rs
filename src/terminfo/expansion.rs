...
use crate::terminfo::util::number;
...
fn operation(input: &[u8]) -> IResult<&[u8], Item<'_>> {
    ...
    match c {
        ...
        b"&" => Ok((
            input,
            Item::Operation(Operation::Binary(Binary::BitwiseAnd)),
        )),
        b"|" => Ok((input, Item::Operation(Operation::Binary(Binary::BitwiseOr)))),
        b"^" => Ok((
            input,
            Item::Operation(Operation::Binary(Binary::BitwiseXor)),
        )),
        b"~" => Ok((input, Item::Operation(Operation::Unary(Unary::BitwiseNot)))),

        b"A" => Ok((
            input,
            Item::Operation(Operation::Binary(Binary::LogicalAnd)),
        )),
