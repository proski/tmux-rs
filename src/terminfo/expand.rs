...
mod test {
    ...
    #[test]
    fn test_op_increment_single() {
        let mut expand_context = ExpandContext::new();
        assert_eq!(
            expand_context.expand(
                b"%p1%d%p2%d%p3%d%i%p1%d%p2%d%p3%d",
                &[Parameter::from(1), Parameter::from(4), Parameter::from(7)],
            ),
            Ok(b"147257".to_vec())
        );
    }
    ...
    #[test]
    fn test_op_increment_repeated() {
        let mut expand_context = ExpandContext::new();
        assert_eq!(
            expand_context.expand(
                b"%p1%d%p2%d%p3%d%i%p1%d%p2%d%p3%d%i%p1%d%p2%d%p3%d",
                &[Parameter::from(1), Parameter::from(4), Parameter::from(7)],
            ),
            Ok(b"147257257".to_vec())
        );
    }

    #[test]
    fn test_op_increment_default_parameters() {
        let mut expand_context = ExpandContext::new();
        assert_eq!(
            expand_context.expand(b"%p1%d%p2%d%p3%d%i%p1%d%p2%d%p3%d", &[]),
            Ok(b"000110".to_vec())
        );
    }
    ...
    fn test_format() {
        ...
        assert_eq!(
            expand_context.expand(b"%p1%:-4.2s", &[Parameter::from(b"hello")]),
            Ok(b"he  ".to_vec())
        );
