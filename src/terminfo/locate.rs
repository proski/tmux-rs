...
fn find_in_directory(term_name: &str, dir: &Path) -> Option<PathBuf> {
    ...
    // directories use the first character of the terminal name in hexadecimal form.
    if let Ok(first_char) = u8::try_from(first_char) {
        let first_char_hex = format!("{first_char:02x}");
